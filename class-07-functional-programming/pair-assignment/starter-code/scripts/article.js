...
(function(module) {
  ...
  // TODO: Chain together a `map` and a `reduce` call to get a rough count of all words in all articles. ***Grabbing Article.all array, map to iterate over every article.  we want to return words from the article body. Return article.something.
  Article.numWordsAll = function() {
    return Article.all.map(function(article) {
      return article.body.match(/\b\w+/g).length;
      // looks for any piece of whitespace, get word count. gives an array of numbers with word counts for each article.
    })
    .reduce(function(a, b) {
  ...
  Article.allAuthors = function() {
    return       // map our collection
      return    // return just the author names (encapsulated)
